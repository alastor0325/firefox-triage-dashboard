...
def test_rail_tag_shows_current_p_s_when_both_set(triage_dir: Path) -> None:
    write_draft(
    ...
    assert "rail-tag--ps" in row
    # The label combines current severity and priority (e.g. "S3·P2").
    assert "S3" in row and "P2" in row


def test_rail_tag_shows_no_ps_when_neither_set(triage_dir: Path) -> None:
    write_draft(
        triage_dir, 1, ni_targets=["x@y"],
        bug_context={},
    )
    body = client.get("/?tab=needs-info").text
    row = _rail_row_for(body, 1)
    assert "rail-tag--no-ps" in row
    assert "no P/S" in row


def test_rail_tag_shows_partial_p_s_as_no_ps(triage_dir: Path) -> None:
    # Treat partial P/S as `no P/S`: until both Bugzilla fields are set the
    # bug is still under-categorised and the user is being nudged. Picking
    # one over `S3·—` keeps the signal binary and avoids visual clutter.
    write_draft(
        triage_dir, 1, ni_targets=["x@y"],
        bug_context={"current_severity": "S3", "current_priority": ""},
    )
    body = client.get("/?tab=needs-info").text
    row = _rail_row_for(body, 1)
    assert "rail-tag--no-ps" in row
    assert "rail-tag--ps" not in row


def test_rail_tag_shows_crash_when_keyword_present(triage_dir: Path) -> None:
    write_draft(
        triage_dir, 1, ni_targets=["x@y"],
        bug_context={"keywords": ["crash", "regression"]},
    )
    body = client.get("/?tab=needs-info").text
    row = _rail_row_for(body, 1)
    assert "rail-tag--crash" in row


def test_rail_tag_shows_crash_when_crash_id_in_description(
    triage_dir: Path,
) -> None:
    write_draft(
        triage_dir, 1, ni_targets=["x@y"],
        bug_context={
            "description_excerpt": (
                "Crashed; bp-12345678-abcd-1234-5678-abcdef012345 shows..."
            ),
        },
    )
    body = client.get("/?tab=needs-info").text
...
def test_rail_tag_crash_keyword_is_case_insensitive(triage_dir: Path) -> None:
    write_draft(
        triage_dir, 1, ni_targets=["x@y"],
        bug_context={"keywords": ["Crash"]},
    )
    body = client.get("/?tab=needs-info").text
    row = _rail_row_for(body, 1)
    assert "rail-tag--crash" in row
